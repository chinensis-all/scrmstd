...
CREATE TABLE IF NOT EXISTS `features` {
    ...
    `updated_at`      BIGINT UNSIGNED     NOT NULL DEFAULT 0 COMMENT '更新时间',
    `deleted_at`      BIGINT UNSIGNED     NOT NULL DEFAULT 0 COMMENT '软删除时间',
    INDEX `idx_parent_id` (`parent_id`) USING BTREE COMMENT '父功能ID索引',
    UNIQUE KEY `uk_feature_name` (`feature_name`) USING BTREE COMMENT '功能名称唯一索引'
) ENGINE=InnoDB AUTO_INCREMENT=1 COMMENT='Sass系统功能表';
