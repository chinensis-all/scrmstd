...
/**
 * ModifyPermissionCommandHandler: 修改权限命令处理器
 */
@Service
public class ModifyPermissionCommandHandler implements CommandHandler<ModifyPermissionCommand, Boolean> {
    private final DomainEventPublisher publisher;
    private final PermissionRepository repository;

    public ModifyPermissionCommandHandler(DomainEventPublisher publisher, PermissionRepository repository) {
    ...
    public Boolean handle(ModifyPermissionCommand command) {
        ...
        aggregate.modify(command.groupId(), command.permissionName(), command.displayName(), command.description());
