...
import com.mayanshe.scrmstd.application.boss.command.ModifyBossAdminCommand;
...
@Service
public class ModifyBossAdminHandler implements CommandHandler<ModifyBossAdminCommand, Boolean> {
    private final BossAdminRepository repository;

    private final DomainEventPublisher publisher;

    public ModifyBossAdminHandler(BossAdminRepository repository, DomainEventPublisher publisher) {
    ...
    @Override
    @Transactional
    public Boolean handle(ModifyBossAdminCommand command) {
        BossAdmin aggregate = repository.load(command.id())
