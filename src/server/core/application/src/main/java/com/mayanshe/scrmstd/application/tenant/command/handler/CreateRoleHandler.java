...
import com.mayanshe.scrmstd.application.tenant.command.CreateRoleCommand;
...
@Service
public class CreateRoleHandler implements CommandHandler<CreateRoleCommand, IDResponse> {
    ...
    public CreateRoleHandler(IdGenerator idGenerator, DomainEventPublisher publisher, RoleRepository repository) {
    ...
    public IDResponse handle(CreateRoleCommand command) {
        ...
        Role aggregate = new Role();
        aggregate.create(new AggregateId(id, true), command.tenantId(), command.roleName(), command.description());
