/*
 * [ScrmStd] - 通用SCRM系统
 * Copyright (C) [2025] [张西海]
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */
package com.mayanshe.scrmstd.application.tenant.query.handler;

import com.mayanshe.scrmstd.application.QueryHandler;
import com.mayanshe.scrmstd.application.tenant.query.PermissionDetailQuery;
import com.mayanshe.scrmstd.application.tenant.query.dto.PermissionDto;
import com.mayanshe.scrmstd.application.tenant.query.repo.PermissionQueryRepository;
import com.mayanshe.scrmstd.shared.exception.ResourceNotFoundException;
import org.springframework.stereotype.Service;

/**
 * PermissionDetailQueryHandler: 获取权限详情查询处理器
 */
@Service
public class PermissionDetailQueryHandler implements QueryHandler<PermissionDetailQuery, PermissionDto> {
    private final PermissionQueryRepository repository;

    public PermissionDetailQueryHandler(PermissionQueryRepository repository) {
        this.repository = repository;
    }

    @Override
    public PermissionDto handle(PermissionDetailQuery query) {
        return repository.single(query.id())
                .orElseThrow(() -> new ResourceNotFoundException("Permission not found"));
    }
}
