 ...
 * @date: 2025-11-16
 * @version: 1.0
 * @description: GetPermissionPaginationHandler: 获取权限分页查询处理器
 */
@Service
public class GetPermissionPaginationHandler implements QueryHandler<PermissionPaginationQuery, Pagination<PermissionDto>> {
    ...
    public GetPermissionPaginationHandler(PermissionQueryRepository repository) {
    ...
    public Pagination<PermissionDto> handle(PermissionPaginationQuery query) {
        ...
        if (query.deleted() != null) {
            criteria.put("deleted", query.deleted());
        }
